...
This workspace now includes `noname/learning-mode.html`, the Lavish-inspired launcher for the integrated Learning Mode. It supports English-only, Chinese-only, and bilingual display modes, then opens the original engine with the native Learning Mode rules and AI. The earlier standalone simulator is preserved as `noname/learning-mode-standalone.html` for reference.
...
The original PWA deployment is now included under `noname/`. Its native engine has a `Learning Mode · 学习模式` entry alongside the original modes. That entry reuses the original 1v1 rules, card definitions, turn flow, and AI through `noname/mode/learning.js`; `noname/learning-mode.html` is the guided Lavish-style entry point that launches it.
